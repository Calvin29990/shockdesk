...
| **Atelier 6** | Butterfly | Plafond de risque | ✅ Fait · 30/08 | `butterfly-peak`, 1 M$, 07-01→08-28 : P&L ≈ **−0,06 %**, drawdown **−0,18 %** strictement borné (amplitude réelle +18,5 % vs +5 % prévu). Le plafond de risque du butterfly est validé : on perd la prime, pas plus. |
...
| **Atelier 8** | Iron Condor | Carry de prime | ✅ Fait · 30/08 | 1 M$ (données réelles) : **+15 132 $ (+1,52 %)**, 60 trades, commissions ≈ 0,3 % du volume (correctif temps réel : 4,21 $/jambe). **alpha −11,70 %** contre SPY +13,21 % : le condor vend le temps, pas la direction — il ne bat pas un marché qui monte de 13 %. ⚠️ Correctif appliqué : le garde-fou `EXIT_IF_FORECAST` surveillait `SPY` sans prévision → no-op. Il surveille désormais `^GSPC` aussi. |
| **Atelier 9** | Révision $r_2$ | Onglet Anticipation | ✅ Fait · 30/08 | POST `/api/ledger/<id>/revision` répond et ajoute une révision datée (`grid` dérivée, `peak_base` 9,0) **sans réécrire l'historique** (`r1` reste lisible). Vérifié par API puis restauration du registre d'origine. |
| **Atelier 10** | CLI Revue | Revue mensuelle | ✅ Fait · 30/08 | `cli revue --name global-macro --asof 2026-08-28` sort les 4 sections (score 5/6, révisions à écrire, calibration, stratégies). ✅ Bonus : le correctif `EXIT_IF_FORECAST` déclenche bien une sortie « prévision de choc active » (visible dans le journal). |
